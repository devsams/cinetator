        ...
        return (
          <div key={d.shoot_day_id} style={dayBox}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              {r && <span style={{ fontSize: 12, color: "#74777f" }}>{r.responded_count}/{r.total} responded</span>}
            </div>
            {d.coordinator_name && (
              <div style={{ fontSize: 12, color: "#74777f", marginTop: 2 }}>Location coordinator: {d.coordinator_name}</div>
            )}
                  ...
                  {r.rows.map((row) => (
                    <div key={row.person_id} style={rosterRow}>
                      <span>
                        ...
                        {" "}{row.name}
                        <span style={{ color: "#74777f", fontSize: 11 }}> · {row.role_type}</span>
                      </span>
            ...
            {d.locked_date ? (
              ...
              <div style={{ marginTop: 12, display: "grid", gap: 8 }}>
                ...
                {d.candidate_dates.length === 0 && <div style={{ color: "#74777f", fontSize: 13 }}>No candidate dates set.</div>}
                {d.tally.map((t) => {
                    ...
                    <div key={t.date} style={{ ...dateRow, ...(isRec ? recRow : {}), ...(locBad ? badRow : {}) }}>
                      <div>
                        ...
                        <div style={{ fontSize: 12, marginTop: 2 }}>
                          ...
                          {t.location_available === null && <span style={{ color: "#74777f" }}>Location: awaiting response</span>}
                        </div>
                      </div>
...
const dayBox = { border: "1px solid #2a2a2e", borderRadius: 10, padding: 14, marginTop: 14 };
const rosterRow = { display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 13, padding: "3px 0" };
const dateRow = { display: "flex", justifyContent: "space-between", alignItems: "center", padding: 12, border: "1px solid #2a2a2e", borderRadius: 8 };
const recRow = { borderColor: "#181", background: "#12281c" };
const badRow = { opacity: 0.7, background: "#2a1414" };
const recTag = { marginLeft: 8, fontSize: 11, fontWeight: 700, color: "#fff", background: "#181", borderRadius: 6, padding: "2px 6px" };
const lockedBanner = { display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 10, padding: 12, background: "#12281c", border: "1px solid #bde5c8", borderRadius: 8, fontSize: 14 };
const lockBtn = { padding: "8px 14px", background: "#f5c518", color: "#0d0d0e", border: "none", borderRadius: 8, cursor: "pointer", fontSize: 13 };
const disabledBtn = { background: "#ccc", cursor: "not-allowed" };
const ghost = { padding: "6px 12px", background: "#161618", border: "1px solid #35353b", borderRadius: 8, cursor: "pointer", fontSize: 13 };
const fieldLabel = { fontSize: 11, fontWeight: 700, color: "#74777f", textTransform: "uppercase", letterSpacing: 0.4, marginBottom: 4 };
